...
export class LexerATNSimulator extends ATNSimulator {
	...
	public match(input: CharStream, mode: number): number {
		...
		try {
			...
			const s0: DFAState | undefined = this.atn.modeToDFA[mode].s0;
			if (s0 == null) {
	...
	protected matchATN(input: CharStream): number {
		const startState: ATNState = this.atn.modeToStartState[this.mode];

		if (LexerATNSimulator.debug) {
			console.log(`matchATN mode ${this.mode} start: ${startState}`);
		}

		const old_mode: number = this.mode;

		const s0_closure: ATNConfigSet = this.computeStartState(input, startState);
		const suppressEdge: boolean = s0_closure.hasSemanticContext;
		if (suppressEdge) {
		...
		if (!suppressEdge) {
			const dfa = this.atn.modeToDFA[this.mode];
			if (!dfa.s0) {
				dfa.s0 = next;
			} else {
				next = dfa.s0;
		...
		const predict: number = this.execATN(input, next);
	...
	protected getExistingTargetState(s: DFAState, t: number): DFAState | undefined {
		const target: DFAState | undefined = s.getTarget(t);
	...
	protected computeTargetState(input: CharStream, s: DFAState, t: number): DFAState {
		const reach: ATNConfigSet = new OrderedATNConfigSet();
	...
	protected failOrAccept(
		...
		if (prevAccept.dfaState != null) {
			const lexerActionExecutor: LexerActionExecutor | undefined = prevAccept.dfaState.lexerActionExecutor;
	...
	protected getReachableConfigSet(input: CharStream, closure: ATNConfigSet, reach: ATNConfigSet, t: number): void {
		...
		for (const c of closure) {
			const currentAltReachedAcceptState: boolean = c.alt === skipAlt;
			...
			for (let ti = 0; ti < n; ti++) {               // for each optimized transition
				...
				const target: ATNState | undefined = this.getReachableTarget(trans, t);
				if (target != null) {
					...
					const treatEofAsEpsilon: boolean = t === IntStream.EOF;
	...
	protected accept(
		input: CharStream, lexerActionExecutor: LexerActionExecutor | undefined,
	...
	protected computeStartState(
		...
		const initialContext: PredictionContext = PredictionContext.EMPTY_FULL;
		const configs: ATNConfigSet = new OrderedATNConfigSet();
		for (let i = 0; i < p.numberOfTransitions; i++) {
			const target: ATNState = p.transition(i).target;
			const c: ATNConfig = ATNConfig.create(target, i + 1, initialContext);
	...
	protected closure(input: CharStream, config: ATNConfig, configs: ATNConfigSet, currentAltReachedAcceptState: boolean, speculative: boolean, treatEofAsEpsilon: boolean): boolean {
		...
		if (config.state instanceof RuleStopState) {
			...
			const context: PredictionContext = config.context;
			if (context.isEmpty) {
				configs.add(config);
				return true;
			}
			...
			for (let i = 0; i < context.size; i++) {
				const returnStateNumber: number = context.getReturnState(i);
				if (returnStateNumber === PredictionContext.EMPTY_FULL_STATE_KEY) {
					continue;
				}

				const newContext: PredictionContext = context.getParent(i); // "pop" return state
				const returnState: ATNState = this.atn.states[returnStateNumber];
				const c: ATNConfig = config.transform(returnState, false, newContext);
		...
		for (let i = 0; i < p.numberOfOptimizedTransitions; i++) {
			...
			const c: ATNConfig | undefined = this.getEpsilonTarget(input, config, t, configs, speculative, treatEofAsEpsilon);
			if (c != null) {
				currentAltReachedAcceptState = this.closure(input, c, configs, currentAltReachedAcceptState, speculative, treatEofAsEpsilon);
			}
		}
		...
		switch (t.serializationType) {
			case TransitionType.RULE: {
				...
				if (this.optimize_tail_calls && ruleTransition.optimizedTailCall && !config.context.hasEmpty) {
					...
					const newContext: PredictionContext = config.context.getChild(ruleTransition.followState.stateNumber);
			...
			case TransitionType.ACTION:
				if (config.context.hasEmpty) {
					...
					// the split operation.
					const lexerActionExecutor: LexerActionExecutor = LexerActionExecutor.append(config.lexerActionExecutor, this.atn.lexerActions[(t as ActionTransition).actionIndex]);
	...
	protected evaluatePredicate(input: CharStream, ruleIndex: number, predIndex: number, speculative: boolean): boolean {
		...
		const savedCharPositionInLine: number = this._charPositionInLine;
		const savedLine: number = this._line;
		const index: number = input.index;
		const marker: number = input.mark();
		try {
			this.consume(input);
			return this.recog.sempred(undefined, ruleIndex, predIndex);
		}
	...
	protected captureSimState(
		settings: LexerATNSimulator.SimState,
		input: CharStream,
	...
	protected addDFAEdge(p: DFAState, t: number, q: ATNConfigSet | DFAState): DFAState | void {
		if (q instanceof ATNConfigSet) {
			...
			*/
			const suppressEdge: boolean = q.hasSemanticContext;
			if (suppressEdge) {
				q.hasSemanticContext = false;
			}

			// @NotNull
			const to: DFAState = this.addDFAState(q);
	...
	protected addDFAState(configs: ATNConfigSet): DFAState {
		...
		const proposed: DFAState = new DFAState(configs);
		const existing: DFAState | undefined = this.atn.modeToDFA[this.mode].states.get(proposed);
		if (existing != null) {
			return existing;
		}

		configs.optimizeConfigs(this);
		const newState: DFAState = new DFAState(configs.clone(true));
		...
		if (firstConfigWithRuleStopState != null) {
			const prediction: number = this.atn.ruleToTokenType[firstConfigWithRuleStopState.state.ruleIndex];
			const lexerActionExecutor: LexerActionExecutor | undefined = firstConfigWithRuleStopState.lexerActionExecutor;
	...
	public consume(input: CharStream): void {
		const curChar: number = input.LA(1);
		if (curChar === "\n".charCodeAt(0)) {
			this._line++;
			this._charPositionInLine = 0;
		} else {
