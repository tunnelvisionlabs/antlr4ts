...
import { MurmurHash } from "../internal";
 ...
 */
function getIndex(bitNumber: number): number {
	return bitNumber >>> 4;
}
...
function unIndex(n: number): number {
	return n * 16;
}
 ...
 */
function findLSBSet(word: number): number {
	let bit = 1;
	for (let i = 0; i < 16; i++) {
		if ((word & bit) !== 0) {
			return i;
...
function findMSBSet(word: number): number {
	let bit = (1 << 15) >>> 0;
	for (let i = 15; i >= 0; i--) {
		if ((word & bit) !== 0) {
			return i;
...
export class BitSet implements Iterable<number>{
	...
	*/
	...
	private _setBits(word: number, value: boolean, mask: number): void {
		if (value) {
			this.data[word] |= mask;
		} else {
